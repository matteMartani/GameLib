    ...
    @section('breadcrumb')
        <nav class="main-breadcrumb" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a class="collegamento-scuro" href="{{route('forum.posts', ['disc_id'=>$discussion->disc_id])}}">{{$discussion->titolo}}</a></li>
                <li class="breadcrumb-item text-white" aria-current="page">
                    {{trans('labels.addpost')}}
                </li>
            </ol>
        </nav>
    @endsection
            ...
            <div class="row py-lg-5">
                <div class="col-lg-6 col-md-8 mx-auto">
                    <h1 class="fw-light text-secondary font-scritte ">
                        {{trans('labels.addpost2')}}
                    </h1>

                    <form class="form-horizontal" name="content-form" method="post" action="{{route('forum.storePost', ['disc_id'=>$discussion->disc_id])}}">

                        @csrf

                        <div class="form-group mx-sm-5 mb-2">
                            <label for="content">{{trans('labels.content')}}</label>
                            <textarea class="form-control" placeholder="{{trans('labels.content')}}" name="content" id="content" rows="3"></textarea>
                            <span id="invalid_content" class="text-danger" ></span>
                        </div>

                        <br>
                        <div class="form-group">
                                <button type="submit" class="btn btn-success" onclick="event.preventDefault(); checkPost()">{{trans('labels.add')}} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-node-plus-fill" viewBox="0 0 16 16">
                                        <path d="M11 13a5 5 0 1 0-4.975-5.5H4A1.5 1.5 0 0 0 2.5 6h-1A1.5 1.5 0 0 0 0 7.5v1A1.5 1.5 0 0 0 1.5 10h1A1.5 1.5 0 0 0 4 8.5h2.025A5 5 0 0 0 11 13zm.5-7.5v2h2a.5.5 0 0 1 0 1h-2v2a.5.5 0 0 1-1 0v-2h-2a.5.5 0 0 1 0-1h2v-2a.5.5 0 0 1 1 0z"/>
                                    </svg></button>
                                <input id="mySubmit" type ="hidden" type="submit" value=\'Add\'/>
                            <a role="button" class="btn btn-danger" href="{{route('forum.posts', ['disc_id'=>$discussion->disc_id])}}">{{trans('labels.cancel')}} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-octagon-fill" viewBox="0 0 16 16">

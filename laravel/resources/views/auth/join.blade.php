    ...
    @section('breadcrumb')
        <nav class="main-breadcrumb" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item text-white" aria-current="page">{{trans('labels.join')}}</li></ol>
        </nav>
    @endsection
...
<section class="py-5 text-center container fontoriginale bg-light main-section">
    <div class="row py-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto">
            <h1 class="fw-light text-muted font-scritte">{{trans('labels.join2')}}</h1>
            <p>
            <span id="invalid_login" class="text-danger" >
                        @if(isset($_SESSION['join_error']))
                    @if($_SESSION['join_error'])
                        There's already a user with this username.
                    @endif
                @endif
                    </span>
            <form id="join" name="join" action="{{route('user.register')}}" method="post">
                @csrf
                ...
                <div class="form-group">
                    <label>{{trans('labels.password2')}}</label>
                    <input type="password" name = "confirm-password" id="confirm-password" class="form-control" placeholder="{{trans('labels.password2')}}">
                    <span id="invalid_password_confirm" class="text-danger" ></span>
                </div>

                <div class="form-group">
                    <label>{{trans('labels.email')}}</label>
                    <input type="email" name = "email" id="email" class="form-control" placeholder="{{trans('labels.email')}}">
                ...
                <div class="form-group d-grid gap-2 d-md-block">
                    <button type="submit" onclick="event.preventDefault(); checkJoin()" class="btn btn-viola mr-1">{{trans('labels.register2')}} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                        </svg></button>
                    <a class="btn btn-secondary" href="{{route('user.login')}}">{{trans('labels.login2')}} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-in-right" viewBox="0 0 16 16">
